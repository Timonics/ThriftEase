...
import SubSportsCat from "./SubSportsCat";
...
const SportsCat = (props: Props) => {
  const api = config.dbURL;
  const [categoryProducts, setCategoryProducts] = useState<Product[]>([]);

  const getSportProducts = async () => {
    try {
  ...
  const sportProductsElements = categoryProducts.map((product) => (
  ...
  return (
    <div className="flex flex-col gap-7">
      <SubSportsCat />
      <div className="flex p-4 gap-10">{sportProductsElements}</div>
    </div>
  );
};
